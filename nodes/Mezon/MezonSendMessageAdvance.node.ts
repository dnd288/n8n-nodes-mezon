import { ApiMessageMention, ChannelMessageContent, MezonClient } from 'mezon-sdk';
...
export class MezonSendMessageAdvance implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Mezon Send Message Advance',
		name: 'mezonSendMessageAdvance',
		icon: 'file:mezon.svg',
		group: ['action'],
		version: 1,
		subtitle: '',
		...
		properties: [
			...
			{
				displayName: 'Channel Message Content',
				name: 'content',
				type: 'json',
				default: '',
			},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (var i = 0; i < length; i++) {
			...
			await client.sendMessage(
				...
				this.getNodeParameter('content', i, '') as ChannelMessageContent,
				this.getNodeParameter('mentions', i, '') as ApiMessageMention[],
				this.getNodeParameter('attachments', i, '') as ApiMessageMention[],
				[
					ref,
				]
			);
		...
		console.log("n8n-nodes-mezon: Send Message advance success, disconnect the client");
		client.closeSocket();
		return returnData;
	}
}
